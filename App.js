...
const TaskDetailModal = ({ task, data, onClose, onUpdateStatus, onDelete, onEdit, isWorker, currentWorkerId, onSendRequest }) => {
  ...
  const [confirmStatus, setConfirmStatus] = useState(null); // {status}
        ...
        <div style={{ marginBottom:14 }}>
            ...
            <div style={{ background:"rgba(52,211,153,0.05)", border:"1px solid rgba(52,211,153,0.18)", borderRadius:10, padding:"12px 14px", marginBottom:10 }}>
              ...
              {!showReqForm ? (
                ...
                <div style={{ marginTop:8 }}>
                  ...
                  <div style={{ display:"flex", gap:7 }}>
                    <Btn label="Send to Admin" icon={Send} small onClick={()=>{if(reqReason.trim()){onSendRequest(liveTask.id,"Done",reqReason);setShowReqForm(false);setReqReason("");}}}/>
          ...
          {showReqForm && liveTask.status!=="In Progress" && (
            <div style={{ marginTop:9, background:"rgba(124,95,245,0.05)", border:"1px solid rgba(124,95,245,0.15)", borderRadius:9, padding:"11px 13px" }}>
              ...
              <div style={{ display:"flex", gap:7 }}>
                <Btn label="Send Request" icon={Send} small onClick={()=>{if(reqReason.trim()){onSendRequest(liveTask.id,reqStatus,reqReason);setShowReqForm(false);setReqReason("");}}}/>
...
// ─── SIDEBAR ─────────────────────────────────────────────────────────────────
const Sidebar = ({ nav, navigate, data, globalTypeFilter, setGlobalTypeFilter, open, setOpen, isAdmin }) => {
  const { clients, sheetTypes } = data;
...
const AdminHome = ({ data, navigate, openModal, currentUser, setGlobalTypeFilter, setData }) => {
  ...
  return (
    <div>
      ...
      {/* Row 3: Special project + admin task + account cases metrics */}
      <SL label="My Workspace" />
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        <MB label="Special Projects" value={specialProjects.length} accent="#ec4899" Icon={Folder}
          onClick={() => document.getElementById("sp-box")?.scrollIntoView({ behavior: "smooth" })} sub="tap to scroll" />
        <MB label="My Task Log"  value={adminTasks.length} accent="#a78bfa" Icon={CheckSquare}
          onClick={() => document.getElementById("at-box")?.scrollIntoView({ behavior: "smooth" })} sub="tap to scroll" />
        <MB label="Account Cases" value={accountCases.length} accent="#fbbf24" Icon={Building2}
          onClick={() => document.getElementById("ac-box")?.scrollIntoView({ behavior: "smooth" })} sub="tap to scroll" />
      </div>
                  ...
                  return (
                    <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", background: "#0e0e17", border: "1px solid #181826", borderRadius: 8 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#4a4a62", marginTop: 1 }}>{proj?.name} {worker ? `· ${worker.name}` : ""}{task.deadline ? ` · Due: ${fmtDL(task.deadline)}` : ""}</div>
                      </div>
                      <Badge label={task.status} style={{ ...stStyle(task.status), fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 99 }} />
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8, marginTop: -8 }}>
        <Btn label="View All" variant="ghost" small onClick={() => navigate("clients")} />
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 22 }}>
        {clients.map(c => {
          ...
          return (
            <div key={c.id} style={{ background: "#0c0c14", border: "1px solid #181826", borderRadius: 10 }}>
              <div className="row-item" onClick={() => navigate("client", { clientId: c.id })} style={{ display: "flex", alignItems: "center", gap: 11, padding: "12px 15px", cursor: "pointer" }}>
                <Av text={initials(c.name)} size={34} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#e2e2f0", fontFamily: "'Syne',sans-serif" }}>{c.name}</div>
                  <div style={{ fontSize: 11, color: "#555568", marginTop: 1 }}>{c.industry} · {c.email}</div>
                  <CreatedAt ts={c.createdAt} />
                </div>
                <div style={{ display: "flex", gap: 12, color: "#555568", fontSize: 11, flexShrink: 0 }}>
                  <span><span style={{ color: "#7c5ff5", fontWeight: 700 }}>{accs.length}</span> accts</span>
                  <span><span style={{ color: "#f59e0b", fontWeight: 700 }}>{projs.length}</span> projs</span>
                  <span><span style={{ color: "#3b82f6", fontWeight: 700 }}>{shs.length}</span> sheets</span>
                </div>
                <ChevronRight size={13} color="#222236" />
              </div>
              {/* Admin quick actions */}
              <div style={{ display: "flex", gap: 6, padding: "0 15px 9px" }}>
                <IconBtn icon={Edit3} onClick={() => openModal("editClient", { entity: c })} title="Edit client" />
                <IconBtn icon={Trash2} onClick={() => openModal("confirmDelete", { type: "client", id: c.id, name: c.name })} title="Delete client" color="#7777a0" hoverColor="#f87171" />
              </div>
            </div>
          );
        })}
      ...
      {/* Accounts Overview */}
      <AccountsTaskBox data={data} navigate={navigate} />

      {/* Account Cases — blockchain */}
      <div id="ac-box"><AccountCasesBox data={data} setData={setData} /></div>

      {/* Special Projects */}
      <div id="sp-box"><SpecialProjectsBox data={data} setData={setData} navigate={navigate} openModal={openModal} /></div>

      {/* Admin Task Log */}
      <div id="at-box"><AdminTaskLog data={data} setData={setData} /></div>
    </div>
  );
};
...
const StatusRequestsPage = ({ requests, data, onApprove, onReject }) => {
  const { workers, tasks } = data;
  return (
    <div className="fade-up">
      <div style={{ marginBottom:18 }}><h1 style={{ fontSize:20, fontWeight:800, color:"#e4e4f4", fontFamily:"'Syne',sans-serif" }}>Status Requests</h1><p style={{ fontSize:12, color:"#666680", marginTop:3 }}>{requests.filter(r=>r.status==="pending").length} pending approval</p></div>
      {requests.length===0 ? <Empty icon={Send} msg="No status change requests"/> : (
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
          {requests.slice().reverse().map(req=>{
            const w=workers.find(x=>x.id===req.workerId);
            const t=tasks.find(x=>x.id===req.taskId);
            return (
              <div key={req.id} style={{ background:"#0c0c14", border:`1px solid ${req.status==="pending"?"rgba(124,95,245,0.2)":"#181826"}`, borderRadius:11, padding:"14px 16px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:9 }}>
                  <div>
                    <div style={{ fontSize:13, fontWeight:600, color:"#e2e2f0" }}>{t?.title||"Unknown task"}</div>
                    <div style={{ fontSize:11, color:"#7777a0", marginTop:2 }}>
                      Requested by: <span style={{ color:"#00c896" }}>{w?.name}</span> · Change to: <Badge label={req.requestedStatus} style={{ ...stStyle(req.requestedStatus), fontSize:10, fontWeight:600, padding:"1px 7px", borderRadius:99 }}/>
                    </div>
                    <div style={{ fontSize:10, color:"#4a4a62", marginTop:3 }}>{fmtTime(req.timestamp)}</div>
                  </div>
                  <Badge label={req.status==="pending"?"Pending Review":req.status==="approved"?"Approved":"Rejected"} style={req.status==="pending"?{...stStyle("Pending"),fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:99}:req.status==="approved"?{...stStyle("Done"),fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:99}:{color:"#f87171",background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.15)",fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:99}}/>
                </div>
                <div style={{ background:"#0a0a12", border:"1px solid #181826", borderRadius:7, padding:"8px 11px", fontSize:12, color:"#7777a0", marginBottom:req.status==="pending"?11:0, lineHeight:1.5 }}>{req.reason}</div>
                {req.status==="pending" && (
                  <div style={{ display:"flex", gap:8 }}>
                    <Btn label="Approve" variant="green" small icon={CheckCircle} onClick={()=>onApprove(req)}/>
...
export default function App() {
  ...
  const renderView = () => {
    ...
    if(view==="requests")      return <StatusRequestsPage requests={data.statusRequests} data={data} onApprove={handleApproveRequest} onReject={handleRejectRequest}/>;
  ...
  return (
    <>
      ...
      <div style={{ display:"flex", height:"100vh", background:"#07070c", color:"#e2e2f0", fontFamily:"'Outfit',sans-serif", overflow:"hidden" }}>
        ...
        <Sidebar nav={nav} navigate={navigate} data={data} globalTypeFilter={globalTypeFilter} setGlobalTypeFilter={setGlobalTypeFilter} open={sideOpen} setOpen={setSideOpen} isAdmin={isAdmin}/>
